    ...
    for novel in st.session_state.api_search_results:
        ...
        with st.container(border=True):
            ...
            if st.button("이 작품 서재에 추가", key=f"add_{novel.get('ncode')}"):
                with st.spinner(f"'{novel.get('title', '...')}' 정보 스크래핑 중..."):
...
ncode_to_add = st.text_input("추가할 작품의 N코드를 입력하세요:", placeholder="예: n2267be")
if st.button("서재에 추가"):
    ncode = ncode_to_add.strip().lower()
    if ncode and re.match(r"^n[a-z0-9]+$", ncode):
...
if not paged_novels:
    ...
    for novel in paged_novels:
        with st.container(border=True):
            col_info, col_buttons = st.columns([0.6, 0.4])
            with col_info:
            ...
            with col_buttons:
                if st.button("챕터 보기", key=f"chapters_{novel['id']}", use_container_width=True):
                ...
                if b_col1.button("갱신", key=f"update_{novel['id']}", use_container_width=True):
                ...
                if b_col2.button("삭제", key=f"delete_{novel['id']}", use_container_width=True, type="primary"):
