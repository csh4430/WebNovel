...
export default function Home() {
  ...
  const handleDeleteNovel = async (novelId: number, novelTitle: string) => {
    if (!window.confirm(`'${novelTitle}' 소설을 정말로 삭제하시겠습니까?`)) return;
    try {
      const res = await fetch(`${API_URL}/api/novels/${novelId}`, { method: 'DELETE' });
  ...
  const handleUpdateNovel = async (novelId: number, novelTitle: string) => {
    ...
    setMessage('');
    try {
      const res = await fetch(`${API_URL}/api/novels/${novelId}/update`, { method: 'POST' });
      const result = await res.json();
    ...
    <main className="min-h-screen">
      <div className="max-w-4xl mx-auto p-8">
        <div className="flex justify-center items-center gap-4 mb-8">
          ...
          <button onClick={() => fetchNovels()} disabled={loading} className="p-2 text-muted-foreground hover:text-foreground" aria-label="Refresh list">
            <RefreshCw className={loading ? 'animate-spin' : ''} />
          </button>
        </div>
        <form onSubmit={handleSearch} className="flex gap-2 mb-4">
          <div className="relative flex-grow">
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="찾고 싶은 소설 제목을 입력하세요..." className="w-full bg-muted text-foreground border border-border rounded-md px-4 py-2 pr-10" />
          ...
          <button type="submit" disabled={isSearching} className="bg-primary text-primary-foreground px-6 py-2 rounded-md flex items-center gap-2">
            ...
            <ul className="space-y-2">
              {searchResults.map(novel => (
                <li key={novel.ncode} className="border border-border rounded-lg p-3 bg-card flex justify-between items-center">
                  ...
                  <button onClick={() => handleAddNovelFromSearch(novel.ncode)} disabled={isAdding} className="bg-primary text-primary-foreground px-4 py-1 text-sm rounded-md">추가</button>
                </li>
              ))}
            </ul>
          </div>
        ...
        {loading ? (
          <ul className="space-y-4">{Array.from({ length: 5 }).map((_, index) => <li key={index}><SkeletonCard /></li>)}</ul>
        ) : (
          <ul className="space-y-4">
            {novels.map((novel) => (
              <li key={novel.id} className="border border-border rounded-lg p-4 bg-card hover:bg-muted/50 flex justify-between items-center">
                ...
                <div className="flex items-center">
                  <button onClick={() => handleUpdateNovel(novel.id, novel.title)} disabled={!!updatingNovelId} className="p-2 text-muted-foreground hover:text-primary"><RefreshCw size={20} className={updatingNovelId === novel.id ? 'animate-spin' : ''} /></button>
                  <button onClick={() => handleDeleteNovel(novel.id, novel.title)} disabled={!!updatingNovelId} className="p-2 text-muted-foreground hover:text-red-500"><Trash2 size={20} /></button>
                </div>
              </li>
            ))}
          </ul>
