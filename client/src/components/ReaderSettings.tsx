...
export default function ReaderSettings({
  ...
  return (
    ...
    >
      <div 
        // 💡 [수정된 부분] p-4 대신 px-4 pt-8 pb-4 를 사용하여 상단 패딩을 늘립니다.
      ...
      >
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          {editing === 'fontSize' ? (
            <input
              ...
              className="w-12 text-right bg-muted/50 rounded-md px-1"
              autoFocus
            />
          ) : (
            <span onClick={() => setEditing('fontSize')} className="w-12 text-right cursor-pointer">
              {fontSize}px
            </span>
          )}
        </div>
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          {editing === 'lineHeight' ? (
            ...
            <span onClick={() => setEditing('lineHeight')} className="w-12 text-right cursor-pointer">
              {lineHeight}
            </span>
          )}
        </div>
        <button onClick={onClose} className="md:hidden absolute top-2 right-2 p-1 text-muted-foreground">
          <X size={20} />
        </button>
      </div>
    </div>
